...
5. 표현 방식 (매우 중요):
   ...
   - 코드나 명령어는 \`\`\`block\`\`\` 로 표기
6. **요점 정리 필수**: 본문이 끝난 후 반드시 "**요점 정리:**" 섹션을 추가하고, 핵심 내용을 가운뎃점 불릿 포인트로 2-3줄 정리하세요. 예: "**요점 정리:**\n· 핵심 1\n· 핵심 2\n· 핵심 3"
...
### 반드시 JSON으로만 응답
{
  "slides": [
    {
      "title": "슬라이드 제목",
      "content": "마크다운 본문 (250~350단어로 충분히 상세하고 깊이 있는 내용, 이미지 언급 없이 텍스트와 코드만 사용). 중요 단어는 **굵게**, 중요 문장은 *기울임*으로 표시. 개념 카드는 '**개념명: 전체 설명 문장**' 형식으로 별도 줄에 작성. 가운뎃점(·)은 불릿 포인트처럼 각 문장 앞에 붙여서 사용. 본문 끝에 반드시 '**요점 정리:**' 섹션 추가하여 핵심을 가운뎃점으로 정리. 코드는 \`\`\`block\`\`\`로 표기"
    }
  ],
