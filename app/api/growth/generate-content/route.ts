...
- 각 슬라이드는 235~335단어로 충분히 상세하고 깊이 있는 내용을 제공한다. 개념 설명에 집중하되, 필요할 때만 간단한 예시를 1-2개 포함한다.
- 이론과 개념 중심으로 설명하며, 예시는 개념을 명확히 하는 데 필요한 경우에만 최소한으로 포함한다.
...
- **[필수] 요점 정리 규칙**: 모든 슬라이드는 본문 끝에 반드시 구분선(---)과 "📌 요점 정리:" 섹션을 포함해야 한다. 핵심 내용을 가운뎃점으로 2-4줄 요약한다. 예: "---\n\n**📌 요점 정리:**\n· 핵심 1\n· 핵심 2\n· 핵심 3"
...
2. **슬라이드당 단어 수: 235~335단어로 충분히 상세하고 깊이 있는 내용을 제공하세요. 개념과 이론을 명확히 설명하는 데 집중하고, 예시는 개념 이해에 필수적인 경우에만 1-2개 포함하세요.**
3. 구조: 도입 → 핵심 개념 → 설명 → 적용 흐름으로 연결
4. 금지 표현: "X는 뭘까? X는 X야" 같은 반복, 과도한 감탄사, 표면적 설명, 불필요한 사례 나열
5. 표현 방식 (매우 중요):
   - 존댓말(합니다체) 사용
   ...
   - 이미지 또는 이미지 설명 절대 금지 (텍스트와 코드만 사용, 이미지 관련 언급도 하지 않음)
   - 코드나 명령어는 \`\`\`block\`\`\` 로 표기
6. **[필수] 요점 정리 - 모든 슬라이드에 반드시 포함**: 각 슬라이드의 본문이 끝난 후 구분선(---)을 긋고 "**📌 요점 정리:**" 섹션을 추가하세요. 핵심 내용을 가운뎃점 불릿 포인트로 2-4줄 정리합니다. 이 섹션은 프론트엔드에서 초록색 카드로 강조 표시됩니다. **요점 정리가 없는 슬라이드는 허용되지 않습니다.** 형식 예시:
...
### 반드시 JSON으로만 응답
{
  "slides": [
    {
      "title": "슬라이드 제목",
      "content": "마크다운 본문 (235~335단어로 충분히 상세하고 깊이 있는 내용, 이미지 언급 절대 금지, 텍스트와 코드만 사용). 중요 단어는 **굵게**, 중요 문장은 *기울임*으로 표시. 개념 카드는 '**개념명: 전체 설명 문장**' 형식으로 별도 줄에 작성. 가운뎃점(·)은 불릿 포인트처럼 각 문장 앞에 붙여서 사용. **[필수] 본문 끝에 반드시 구분선(---)과 '📌 요점 정리:' 섹션을 추가하여 핵심을 가운뎃점으로 2-4줄 정리**. 코드는 \`\`\`block\`\`\`로 표기. **절대로 이미지, 그림, 사진, 차트, 다이어그램 등 시각적 요소에 대한 언급이나 설명을 포함하지 않음**"
    }
  ],
...
- **모든 슬라이드는 반드시 "---" 구분선과 "📌 요점 정리:" 섹션으로 끝나야 합니다. 이것은 필수 사항입니다.**`;
}
