...
- **가운뎃점(·) 사용 규칙**: 본문에서 여러 항목을 나열할 때 가운뎃점으로 구분한다. 가운뎃점은 같은 문장 내에서 항목들을 연결할 때 사용한다. 예: "디자인은 깔끔하게 · 기능은 직관적으로 · 퍼포먼스는 빠르게 유지하는 것이 중요하다", "문제 분석 능력 · 협업 커뮤니케이션 · 디테일 체크가 핵심이다"
...
5. 표현 방식 (매우 중요):
   ...
   - **가운뎃점(·) 사용법**: 같은 문장 내에서 여러 항목을 나열할 때 가운뎃점으로 구분합니다. 예: "디자인은 깔끔하게 · 기능은 직관적으로 · 퍼포먼스는 빠르게 유지", "문제 분석 능력 · 협업 커뮤니케이션 · 디테일 체크"
...
### 반드시 JSON으로만 응답
{
  "slides": [
    {
      "title": "슬라이드 제목",
      "content": "마크다운 본문 (250~350단어로 충분히 상세하고 깊이 있는 내용, 이미지 언급 없이 텍스트와 코드만 사용). 중요 단어는 **굵게**, 중요 문장은 *기울임*으로 표시. 개념 카드는 '**개념명: 전체 설명 문장**' 형식으로 별도 줄에 작성. 가운뎃점(·)은 같은 문장 내 여러 항목을 나열할 때 사용 (예: '디자인은 깔끔하게 · 기능은 직관적으로 · 퍼포먼스는 빠르게'). 코드는 \`\`\`block\`\`\`로 표기",
