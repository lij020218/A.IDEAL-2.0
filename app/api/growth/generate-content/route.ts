...
# 가운뎃점(·) 사용 규칙
- **오직 리스트 나열 시에만 사용**: 여러 항목을 나열할 때 각 항목 앞에 가운뎃점을 붙임
- 일반 문단 작성 시에는 절대 사용하지 않음
- 예시:
  디지털 마케팅의 핵심 요소는 다음과 같습니다.

  · 고객 데이터 수집 및 분석을 통한 타겟팅
  · 다양한 디지털 채널을 활용한 캠페인 운영
  · 실시간 성과 측정 및 최적화

# [필수] 요점 정리 규칙
- **모든 슬라이드는 반드시 요점 정리로 끝나야 함**
...
7. **문장 흐름**:
   - 일반 문단은 자연스럽게 2-3문장씩 이어서 작성
   - 문단 사이에 빈 줄을 두어 가독성 향상
   - **가운뎃점(·)은 오직 리스트 나열 시에만 사용** - 일반 설명 문장에는 절대 사용 안 함
   - 리스트 예시:
     디지털 마케팅의 핵심 요소는 다음과 같습니다.
...
8. **[필수] 요점 정리**:
   - **모든 슬라이드는 반드시 요점 정리로 끝나야 함 (이것은 필수입니다)**
   - 본문 끝에 구분선(---)을 긋고 "📌 요점 정리:" 섹션 추가
...
- **모든 슬라이드는 반드시 "---\\n\\n**📌 요점 정리:**\\n· 항목1\\n· 항목2" 형식으로 끝나야 합니다.**
- 가운뎃점(·)은 리스트 나열할 때만 사용하고, 일반 문장에는 사용하지 마세요.
