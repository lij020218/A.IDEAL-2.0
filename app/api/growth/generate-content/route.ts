...
### 반드시 JSON으로만 응답
{
  "slides": [
    {
      "title": "슬라이드 제목",
      "content": "마크다운 본문 (250~350단어로 충분히 상세하고 깊이 있는 내용, 이미지 언급 없이 텍스트와 코드만 사용). 중요 단어는 **굵게**, 중요 문장은 *기울임*으로 표시. 개념 카드는 '**개념명: 전체 설명 문장**' 형식으로 별도 줄에 작성 (예: **디지털 마케팅: 디지털 채널을 활용해 고객을 발견·획득·유지하며, 데이터를 기반으로 최적화하는 활동**). 가운뎃점(·) 사용 시 반드시 다음 줄로 넘기기. 코드는 \`\`\`block\`\`\`로 표기",
