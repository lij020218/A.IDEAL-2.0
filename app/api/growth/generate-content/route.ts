...
- "개념: 설명" 형식의 핵심 개념은 별도 줄에 작성한다. 이는 프론트엔드에서 파란 카드로 표시된다. 단순 중요 단어는 굵게 처리만 한다.
- 본문에서 가운뎃점(·)을 사용할 때는 반드시 다음 줄로 넘기고, 가운뎃점이 항상 그 줄의 시작이 되도록 한다.
...
5. 표현 방식 (매우 중요):
   ...
   - **"개념: 설명" 형식일 때만 파란 카드로 표시됨. 반드시 별도 줄에 작성하세요.** 예: "**트랜잭션: 데이터베이스 작업의 논리적 단위**"
   - 단순 중요 단어는 굵게 처리만 하고 카드로 만들지 마세요
   - **본문에 가운뎃점(·) 사용 시 반드시 다음 줄로 넘기세요.** 가운뎃점은 항상 줄의 시작에 위치해야 합니다
...
### 반드시 JSON으로만 응답
{
  "slides": [
    {
      "title": "슬라이드 제목",
      "content": "마크다운 본문 (250~350단어로 충분히 상세하고 깊이 있는 내용, 이미지 언급 없이 텍스트와 코드만 사용). 중요 단어는 **굵게**, 중요 문장은 *기울임*으로 표시. '개념: 설명' 형식은 별도 줄에 작성. 가운뎃점(·) 사용 시 반드시 다음 줄로 넘기기. 코드는 \`\`\`block\`\`\`로 표기",
